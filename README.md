...
一个 AI 驱动的文字冒险游戏：新建时选择故事卡，AI 根据该卡的世界、人物与规则生成开场；你输入想做的事，AI 续写剧情。内置修仙「问道 · 玄苍大陆」和科幻「星港余波」。

## 玩法

- 点击「新建故事」，选择故事卡后开始。每张卡包含独立提示词、地图、人物、势力、物品及初始状态。
- 你在输入框自由输入任何行动（不是选项菜单）。
- AI 根据世界逻辑推进剧情，选择有真实后果。
- 网页端流式打字机效果，边生成边显示。「新建故事」可重新选卡开局。
- **自动存档**：每一手都写入 SQLite，重启不丢。点「存档」可查看所有故事、读档重放、改名或删除。启动时自动续上最近一局。
- **用户隔离**：访问页面后必须先输入 `userid`，每个用户只会看到和操作自己的存档；已有存档使用 `default` 进入。
...
- **长期爽点**：爽点 Agent 在多轮之间维护一个尚未触发的高价值机缘或突破，并按剧情为现有世界机会动态关联故事卡中的技能、装备或身份等奖励；只有玩家满足明确条件且正文实际兑现后，异步审计才将其标记为已触发。
...
backend/
  ...
  prompt_templates/  # 按层与 Agent 拆分的通用 Markdown 提示词模板
  story_cards.py     # 世界包加载、校验和卡片目录
  story_cards/       # 每张故事卡一个目录：card.json + 题材 Markdown
frontend/
  index.html   # 页面
  style.css    # 古风暗色主题
...
- 每局另外保存完整的 `story_card` 快照，修改本地卡片只影响之后的新局；旧修仙存档自动补齐故事卡信息，保留原剧情和当前位置。
- 存档通过必填的 `X-User-ID` 请求头隔离，已有存档仍属于 `default` 用户。该机制适合可信环境下的本地多人区分，不提供密码鉴权。
- 故事卡接口：`GET /api/story-cards` 返回可选卡片；`POST /api/new` 接受 `{"story_card_id":"orbital"}`。省略卡片 ID 时兼容旧客户端，默认使用 `xiuxian`。
...
- 通用提示词在 `backend/prompt_templates/`，变量使用 `${name}`；题材提示词与世界数据在 `backend/story_cards/`。修改后需重启后端。见 [提示词维护说明](docs/PROMPTS.md) 和 [故事卡编写说明](docs/STORY_CARDS.md)。

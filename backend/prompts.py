...
def render_system_prompt(name: str, *, context: str = "") -> str:
    """Compose system rules with explicitly positioned context and shared rules."""
    variables = {}
    if name in {"engine/event/system", "engine/causal/system"}:
    ...
    if name in {
        ...
        "engine/event/system",
        "engine/causal/system",
        "guidance/conflict/system",
    }:
        variables["cultivation_rules"] = CULTIVATION_SYSTEM_APPENDIX
    return render_prompt(name, **variables)


# Compatibility names for existing callers and prompt-contract tests.
CULTIVATION_SYSTEM_APPENDIX = render_prompt("shared/cultivation")
